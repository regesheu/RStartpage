...
const RDrive = (() => {
  ...
  function isConfigured() {
    ...
    return !!chrome.identity?.getAuthToken && id.endsWith('.apps.googleusercontent.com') && !/REPLACE|PLACEHOLDER/i.test(id);
  }
  ...
  async function request(path, options = {}, interactive = false, upload = false) {
    const access = await token(interactive);
    const response = await fetch(`${upload ? 'https://www.googleapis.com/upload/drive/v3' : API}${path}`, { ...options, signal: AbortSignal.timeout(60000), headers: { Authorization: `Bearer ${access}`, ...(options.headers || {}) } });
  ...
  async function list() {
    ...
    do {
      const query = new URLSearchParams({ spaces: 'appDataFolder', fields: 'nextPageToken,files(id,name,size,createdTime,modifiedTime)', orderBy: 'createdTime desc', q: 'trashed=false', pageSize: '100', ...(pageToken ? { pageToken } : {}) });
      const page = await request(`/files?${query}`); files.push(...(page.files || [])); pageToken = page.nextPageToken || '';
    } while (pageToken);
    return { files };
  }
  ...
  async function upload(name, value, existingId = '') {
    ...
    return request(path, { method: existingId ? 'PATCH' : 'POST', headers: { 'Content-Type': `multipart/related; boundary=${boundary}` }, body: multipart }, false, true);
  }
